...
const serializeBlock = (block: any) => {
  ...
  switch (block.blockType) {
    ...
    case "linkList":
      return (
        <div key={block.id} className="my-8">
          <h3 className="mb-4 text-xl font-bold">{t("blocks.linkList.title")}</h3>
          <ul className="space-y-2">
            {block.links?.map((link: any) => (
              <li key={link.id}>
                <Link
                  href={link.isExternal ? link.externalUrl : `/articles/${link.internalUrl.slug}`}
...
export const Serialize = ({ content }: { content: any[] }) => {
  if (!content) return null;
  return <>{content.map((block) => serializeBlock(block))}</>;
};
